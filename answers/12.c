...
int main() {
	...
	char *p1_wins_text = "Player 1 Wins!";
	char *p2_wins_text = "Player 2 Wins!";
	int winner = 0;
	...
	while(!WindowShouldClose()) {
		...
		if (app_state == AS_MAIN_MENU) {
			...
			char *results_text;
			if(winner == 1) {
				results_text = p1_wins_text;
			} else {
				results_text = p2_wins_text;
			}
			DrawText(results_text, 10, 10, 40, GOLD);
			DrawText("Press Enter for Main Menu", 10, 60, 30, WHITE);
			EndDrawing();
